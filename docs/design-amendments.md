...
## 2026-09-14g — 基礎牌 5 張（鎖定）

覆寫 **2026-09-13t**「玩家牌庫預設 15／6× 基礎」。

- 每職預設 **14**：5× 基礎（騎 `attack`、槍 `shot`、法 `magic_arrow`）＋ 4 小技×2 ＋ 1 大招。
- 重製仍可勾 0–2 張小技各 +1 → 最多 16。勾兩張不再是 17。


---

## 2026-09-14h — 最短路不預設踩咒（鎖定）

`shortestPath` 若存在**等長**且中途不踩詛咒的路，選那條。只有更長的繞路才不取代（仍受移動上限）。目標本身是詛咒格仍可走。英勇衝鋒不走此函式。

---

## 2026-09-14i — 御風禁咒／嘲諷吸引／奉獻 tutor 不死／位面免疫（鎖定）

> 來源：人類定案 2026-09-14。覆寫 **2026-09-09f** 嘲諷「禁鋪鄰 1」、**2026-09-14f** 奉獻「不再抽出不死」、御風可推 curse、位面免疫未接 UI。  
> （檔內已有 2026-09-14g／h，本條用 **i**。）

### 定案

1. **御風不可推詛咒**  
   `mageWindAllowsKind('curse') === false`（同 silence／punish／mud）。board `canPushFrom` 仍可允許 curse（其他系統用）。

2. **嘲諷改吸引**  
   王必須在騎士鄰 1（`ringDistance`）的空可站格鋪牆；有至少一格有效則整次鋪放只從該環選（仍走威脅序）。環上零有效空格則退回一般威脅序。屏障仍走 `protectedHexes` 擋鋪；嘲諷覆蓋時不把嘲諷環當保護格。`BossPlaceRestriction` 型別不變，語意改為必須鋪。`pickThreatPlacementHexes` 新增 `forcedHexes`。

3. **奉獻咒滿 tutor 不死存在**  
   仍走咒滿鋪鄰＋封印。另從該職**剩餘牌庫**（非棄牌、非手牌）抽出 1 張 `undying` 進手（超手牌上限棄最新）。牌庫沒有則日誌「牌庫沒有不死存在」，不抽其他牌。`extractedUndying`＝咒滿時上層應從牌庫抽出，不是死亡路徑。

4. **位面調換後雙方本回合免疫沉默與封印**  
   成功換位：解封雙方；設 `silenceImmuneThisRound`／`sealImmuneThisRound`（輪末清）。鄰沉默不鎖受沉默牌；`refreshEnclosure` 在 sealImmune 時不重封。不可與自己換（`same_actor`）。增幅仍只讓該張牌不受沉默。日誌：「換位後雙方本回合不受沉默／封印」。

### 給 bot（禁改）

- 不要改 `DEFAULT_MAP_RADIUS`
- 不要讓御風再可推 curse
- 不要把嘲諷改回禁鋪鄰 1
- 不要從棄牌堆 tutor 不死存在
