...
## 2026-09-19 — 牌面／行動序／衝鋒完整停（鎖定）

> 來源：人類定案（cardHints 新稿 + playtest 1–16 + 衝鋒／位面增幅訂正）。  
> **覆寫** 09-09f 衝鋒「穿完整老化牆」、09-13i「首步鎖出牌至走完 2 步」、14i 位面「增幅只改沉默、距離固定 3」、信仰「出牌前不可移動」。

### 定案 — 牌面規則

1. **強能增幅**：不計次；**不受沉默**；效果不帶進下回合。
2. **位面調換**：改**計次**；成功仍強制結束。施法者點牌後**依序選兩個**距離內角色（可含自己，也可選另外兩人）才互換。同一人點兩次＝取消第一目標。不可兩目標相同、不可王。增幅：該張不受沉默／封印，且距離 **+1**（3→4）。不是無限距離。
3. **大鬧一場**：出牌前仍不可移動。成功後可打 1 張手上射擊（不計次、無視距離）。**打完或放棄（結束回合／手中無射擊）才結束回合**。不可穿插其他牌。
4. **御風**：不可推沉默／懲罰／泥濘／**詛咒**（14i 維持）。
5. **堅定信仰**：不再要求出牌前未移動。成功後**剩餘步數清 0**（不可再走）。
6. **磁力屏障**：成功後**強制結束回合**。冷卻（下一自己回合不可再出）維持。
7. **英勇衝鋒**：完整牆（`plain`／`plain_starter`，不論老化）**停前一格並打一下**（只破碎、**不傷王**）。破碎牆撞破續衝。**只有破壞老化牆才傷王**（未老化破壞不傷；牆傷 cap 2 維持）。

### 定案 — 行動序（覆寫 09-13i 移動鎖牌）

- 不可「移動 → 出牌 → 再移動」。移動後再出牌 → 剩餘步數清 0。出牌後再移動可以。
- 首步**不再**鎖出牌。
- 該角色一有移動或出牌（`actedThisTurn`）→ 必須結束回合才能換人。
- 結束回合後自動切到下一個還能行動者。

### 定案 — 指定／時序

- 狂妄推牆：**先推完，王才鋪格**。
- 位面：懸停顯示距離圈；點牌進入指定，再點範圍內友軍（不是一按就換）。
- 御風／不死／屏障範圍要高亮。

---

## 2026-09-19b — 衝鋒破壞才傷／嘲諷改打斷確認（鎖定）

1. **衝鋒**：完整老化牆停下只破碎，**0 傷**。必須把老化牆**破壞**（清空格）才 +1 王傷。
2. **嘲諷**：不再用手牌主動打、不再勾「他人回合」。隊友（非騎士）有效傷王、王要鋪格前：若騎士手上有嘲諷、未出局、未封印 → 騎士棋子上方跳出「嘲諷」與「×」。確定＝耗 1 張嘲諷並強制王鋪鄰 1；×＝這次不用、牌留著。法師打一次、槍手打一次會各問一次。不佔騎士行動、不鎖騎士回合。
3. **代價移動**：自計畫撤下，下一隻魔王之後再議。

---

## 2026-09-19c — 咒滿兩層／破碎外觀／氣瓶增幅提示（鎖定）

1. **咒滿**：一般職業踩到 **2** 層才鋪鄰封印；騎士被動 +1 → **3**。一層只是「身上有咒」，不鋪鄰、不封印。達 cap 當下**清掉身上詛咒層數**（封印來自鋪鄰，不是靠咒層本身）。
2. **詛咒地形**單一紫色；可走範圍不再把咒格洗成另一色。
3. **破碎牆**（`plain_broken`）必須看得出裂痕／「破碎」標，不可與完整一般格同貌。
4. **槍手**隨時顯示裝填槽（傷／抽／推）。**法師**增幅待用時棋子與職業卡都要有「增幅」。
