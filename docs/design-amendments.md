...
> 來源：人類定案「來吧! 大鬧一場! 強化」。  
> **保留**既有：×1；出牌前不可移動（`hasMovedThisTurn` 失敗）；抽 `BIG_SHOW_DRAW`（3）；抽到的非射擊可立刻使用；之後 `mayPlayShot`；計次；受沉默。

### 定案 — 新增

1. **忽略甜區／距離罰（僅此大招開啟的那一發射擊）**  
   - 成功結果帶 `ignoreRangePenaltyForShot: true`（語意同 `mayPlayShotIgnoreRange`）。  
   - 上層打該後續射擊時，必須把旗標傳入 `resolveGunnerShot({ ignoreRangePenalty: true })`，使 `computeRangedDamageToBoss` 跳過區外 −1。  
   - **僅限**大招允許的那一發；一般射擊／Power UP 臨時射擊不自動繼承。

2. **免費氣瓶裝填（不棄射擊、可超 cap）**  
   - 成功時對當前裝填槽 `+BIG_SHOW_FREE_DAMAGE`（頑皮／傷害）與 `+BIG_SHOW_FREE_DRAW`（胡鬧／抽），**不需棄手上射擊**。  
   - **可超過** `AMMO_SLOT_MAX_TOTAL`（2）。實作走 `addAmmoUnchecked` 或 `addAmmo(..., { ignoreCap: true })`。  
   - **一般氣瓶**仍必須走有 cap 的 `addAmmo`（上限 2）；不可被本修訂放寬。  
   - 事件：發 `AmmoSlotLoaded`（裝填後槽總量）與／或 `BigShowAmmoGranted`（本次免費層＋裝填後槽）。

### 峰值示例（基礎射擊傷 1）

| 大招前槽 | 免費後 | 後續射擊（甜區／ignore） | 另：大招抽 |
|---|---|---|---|
| 空 `{0,0}` | `{1,1}` | 2 傷 + 抽 1 | +3 |
| 滿 `{1,1}` | `{2,2}` | **3 傷** + 抽 2 | +3 |
| 偏 `{2,0}` | `{3,1}` | 4 傷 + 抽 1 | +3 |
...
- **常見高潮**：滿槽 `{1,1}` + 免費 → `{2,2}` → 後續射擊 **3 傷 + 抽 2**（另加大招本身抽 3，不計入射擊抽）。
- **單邊峰值**：`{2,0}`+免費 → **4 傷 + 抽 1**；`{0,2}`+免費 → **2 傷 + 抽 3**。二者互斥路徑，勿混成同一包。
- **禁止寫成**同時「三傷＋三抽」為最大組合：射擊抽來自 `drawBonus`，大招抽 3 是另一次抽牌，不可把兩者加成宣傳成一次「三傷＋三抽」峰值包。
